class Game:
  def __init__(self, totalEther, totalToken, creatorToken, winnerEther, winnerToken, loserToken, rightVerifierToken):
    self.etherForCompensation = totalEther * 10**18    # Ether from losers only
    self.rewardTokenPool = totalToken
    self.creatorTokens = creatorToken
    self.winnerEther = winnerEther * 10**18
  ...
  def creator(self, tokenAmount):
    ...
    print("Ether Reward: %f" % (self.etherForCreators * tokenAmount / self.creatorTokens / (10**18)))
    print("Token Reward: %d" % (self.tokenForCreators * tokenAmount / self.creatorTokens))

  def winner(self, tokenAmount, etherAmount):
    print("----- Winner (ether: " + str(etherAmount) + ", token: " + str(tokenAmount) + ") -----")
    etherAmount *= 10**18
    etherReward95 = self.ether95 * etherAmount / self.winnerEther / 10**18
    etherReward5 = self.ether5 * tokenAmount / self.winnerToken / 10**18
    etherRewardTotal = etherReward95 + etherReward5 
    
    print("Ether Reward: %f + %f = %f" % (etherReward95 , etherReward5, etherRewardTotal)) 
  ...
  def verifier(self, tokenAmount):
    ...
    print("Ether Reward: %f" % (self.etherForVerifiers * tokenAmount / self.rightVerifierToken / (10**18))) 
