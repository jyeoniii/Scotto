...
contract Game {
    ...
    event _log (address addr, uint amount);
    event logInt(uint);
    ...
    */
    ...
    function enterResult(address sender, uint tokenAmount, uint8 result) public {
         ...
         Verifier memory verifier = Verifier(msg.sender, tokenAmount, result);
    ...
    function finalize()public returns (uint8[]) {
      ...
      for(uint i = 0; i < finalResult.length; i++){
         uint8 winIdx = finalResult[i];
         initDistribute(winIdx); // give ether back to winners & give token back to right verifiers

         winnerEtherAmount += resultStat[winIdx].totalEtherBetted;
         winnerTokenAmount += resultStat[winIdx].totalTokenBetted;
         rightVerifierTokenAmount + resultStat[winIdx].totalTokenFromVerifiers;

         rewardTokenPool += resultStat[i].totalTokenBetted;
      }

      // Loser result
      for (i=0; i<loseResult.length; ++i) {
        uint8 loseIdx = loseResult[i];

        etherForCompensation += resultStat[loseIdx].totalEtherBetted;
        loserTokenAmount += resultStat[loseIdx].totalTokenBetted;

        rewardTokenPool += resultStat[loseIdx].totalTokenBetted;
        rewardTokenPool += resultStat[loseIdx].totalTokenFromVerifiers;
      }
    ...
    */
    ...
    function initDistribute(uint8 result) public {
        ...
        logInt(participants.length);
        // _log(participants[0], 10);
        // return;
